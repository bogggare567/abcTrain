...
| UI polish beyond the current pass: per-widget hover/press *state interpolation* (press-scale-then-spring-back), gradient-filled spectrum/waveform curves, pill-shaped tooltip backgrounds for guide labels, `FlexBox`-based layout, a light theme, a licensed custom typeface | ⏳ deliberately deferred, see [decisions/009](decisions/009-look-and-feel.md) and [decisions/018](decisions/018-ui-polish-and-builtin-samples.md) |
