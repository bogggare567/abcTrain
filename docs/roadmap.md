...
| Shared visualization component factored out for reuse across Learner plugins | ✓ `shared/analysis/WaveformDisplay.{h,cpp}`, extracted from LearnerComp once LearnerVerb needed the identical shape. `SpectrumAnalyserComponent` (LearnerEQ) is still separate — it's FFT-based, a fundamentally different data shape from the time-domain peak-tracking the other two share, so there's nothing to unify there. |
...
| Unified visualization across Learner plugins: `shared/analysis/SpectrumAnalyzer` (extracted from LearnerEQ), Waveform+Spectrum in all three, Bypass/A-B toggle in all three next to Lesson (see [decisions/006](decisions/006-unified-visualization.md)) | ✓ |
| Downloadable builds: CI uploads a per-OS artifact on every push, publishes a GitHub Release with all three OS builds when a `vX.Y.Z` tag is pushed, and each plugin has a manual "Check for Updates" button (`shared/updates/UpdateChecker`, see [decisions/007](decisions/007-update-checker.md)) | ✓ — no background daily timer, manual only; see the ADR for why |
...
| Localization (i18n): `shared/i18n/LocalisationManager` + a flat JSON string table per language for 12 languages (en/ru/de/fr/es/pt/zh-Hans/ja/ko/it/pl/uk), auto-detected + persisted, with a language picker wired into EarTrainer's editor as the reference integration — see [decisions/011](decisions/011-i18n.md) | ✓ all 12 languages carry every key (804 as of v1.7.0), the Learner plugins included — knobs, guide text, presets, modules; only the trainer's dynamic feedback sentence is still English-only |
...
| LearnerVerb: impulse-response visualization, decay-vs-frequency graph, stereo correlometer/vectorscope | ◐ the **echogram** (the measured impulse response of the current setting, with its RT60) replaced the spectrum in v1.6.0, see [decisions/037](decisions/037-the-learner-plugins-made-honest.md); decay-vs-frequency and a correlometer are still ⏳ |
...
| Per-knob **training modules** — demonstrate, try, then a graded check by ear — over synthesized beds generated per module | ✓ 7 in Learner Comp, 7 in Learner Verb, **4 in Learner EQ** since v1.6.0; each a ten-step 3-down/1-up staircase with the band in the knob's units, see [decisions/027](decisions/027-training-modules.md) and [037](decisions/037-the-learner-plugins-made-honest.md) |
| **The Learner plugins made honest** (v1.6.0): one shared editor shell, A/B slots saved with the project, a reverb engine whose Decay is measured RT60 and whose Size does something, the Comp transfer curve, the family colour on every filled control, the module shelf as a card grid, every plugin string in 12 languages | ✓ see [decisions/037](decisions/037-the-learner-plugins-made-honest.md) |
| **Beginner / Pro** settings and optional **hearing protection** per WHO / ITU-T H.870 in the trainer (v1.6.0) | ✓ see [decisions/036](decisions/036-beginner-pro-and-hearing.md) |
| **The staircase**: levels as 3-down/1-up per exercise, thresholds in the exercise's own units, two-layer achievements (v1.6.0) | ✓ see [decisions/035](decisions/035-the-staircase.md) |
| A **frequency map** for Learner EQ — fundamentals, harmonics and named problem zones per source, drawn on the spectrum it already has | ⏳ the zone labels are the first layer of it; per-source maps not built |
...
| B2B: school/studio accounts, progress tracking, LMS integration | ⏳ — superseded for now by the account-free **Education** direction below: assignments and results as files, no server |
| Sales site + payment integration | ⏳ |

## Education — Training → Teaching → Live

Recorded 2026-09-21. The idea is the user's: offer abcTrain to
sound-engineering schools (Music Head was the first named), because a
teacher can explain what 300 Hz is but a student has to *hear* it, and
later run it live at seminars with the room voting on what they heard.
The full note — the idea, an outside assistant's expansion, where we
disagree with it, and sketches of every screen — is
[design/education-and-live.md](design/education-and-live.md). Where the
code goes is [ADR 039](decisions/039-structure-for-growth.md).

| Step | What | Status |
|---|---|---|
| 0. Pilot, no code | one teacher, today's app: placement run, three weeks × fifteen minutes a day, placement run again. The staircase makes before/after a measurement rather than an opinion | ⏳ — needs a letter, not a build. **Decides whether steps 1–2 are worth building** |
| 1. Result export | the trainer saves a run as a file: exercise, threshold in its units, per-round answers | ⏳ — small; `abc_trainer_engine` already holds all of it |
...
`SettingsScreenComponent` is a side rail — five pages since v1.6.0
(Training, Hearing, Appearance, Background, About) — precisely so more can
be added without another redesign. Adding a page is a value in the `Page` enum, a label, and a
`resized()` branch.
