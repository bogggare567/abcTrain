...
| Third-party UI libraries (`foleys_gui_magic`/`gin`/animation), adopted one library/one component at a time rather than a wholesale editor rewrite — see [decisions/013](decisions/013-ui-libraries.md) | 🚧 Phase 1 done: `EarTrainerEditor`'s level progress bar now eases via JUCE's own `juce_animation` module (the originally-named "juce_animate" library doesn't exist). Phase 2 (`gin`, one component) and Phase 3 (`foleys_gui_magic`, prototype only) not started |
