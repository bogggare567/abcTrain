# Code map — the per-file breakdown

What each file is for and why it has the shape it has. This used to be the
body of `CLAUDE.md`; it moved here so a coding session loads a short
`CLAUDE.md` and reads this only for the part it is changing. The *why* of
each decision is in [decisions/](decisions/); the map is
[orientation.md](orientation.md).

Some older paragraphs describe a first version of something that has since
been rebuilt; where an ADR is newer than the paragraph, the ADR wins.
...
- `Source/Games/CompressionGame.{h,cpp}` — "guess the compression": a
  synthesized drum loop (the exercise's own sound, ADR 040) through
  `shared/dsp/CompressorEngine` — the same soft-knee engine Learner Comp
  runs — at two of three amounts (Weak/Medium/Strong), with the makeup gain
  measured per round on the material actually playing, so loudness is not
  a tell. `setDifficulty` pulls the three presets toward Medium from one
  ramped spread, and raises the chance the pair drawn is two neighbours.
- `Source/Games/ReverbGame.{h,cpp}` — "guess the reverb type": a single
  synthesized hit (ADR 040) in one of 5 spaces rendered by
  `shared/dsp/ReverbEngine` — Learner Verb's engine: Room and Chamber on its
  FDN with short lines, Hall on long lines, Plate on Dattorro's tank, Spring
  on the dispersive loops. Each type has a **family** of 3-4 `Variant`s in
  real units (RT60 s, pre-delay ms, size, damping, width, send level,
  archetypal); the least archetypal member of each leans toward its
  neighbour. Played as a send (dry + wet × send), matched in loudness by a
  measurement on its own engine. `ReverbEngine::setTypeNow` changes space
  between rounds without a crossfade. `confusabilityOf()` places the types
  on a character axis for `drawPair`, with Spring held apart (a mechanism,
  not a size) except against Plate.
...
- `Source/Games/DistortionGame.{h,cpp}` — "guess the distortion type": a
  synthesized chord (ADR 040 — pink noise has no pitch, so no harmonics to
  hear) through one of 4 waveshapers (Soft Clip = tanh, Hard Clip = clamp,
  Tape = tanh + post-shaping lowpass, Overdrive = asymmetric tanh),
  **anti-aliased** with first-order ADAA (`antiderivative`,
  `shapeAntiAliased`; Parker et al., DAFx 2016). Each type has a family of
  4 `Variant`s (drive scale, rolloff, asymmetry). The makeup gain is
  measured on the material actually playing (`measureMakeupFor`, with a
  pink-noise overload the tests pin), because a waveshaper is
  level-dependent. See ADR 031 and ADR 040.
...
- `shared/audio/AudioSliceAnalyzer.{h,cpp}` — cuts an imported file into
  ...
  construction. It does not separate stems (the separator of ADR 038 was
  removed in ADR 040). `ReferenceAudioLibrary::importAndSlice` is the file-writing
...
- `shared/dsp/EQCoefficients.h` (moved in ADR 040) — builds a band's
  coefficients from its type/freq/gain/Q; the bell is Vicanek's **matched**
  design (`makeMatchedBell`), which keeps its shape near Nyquist where RBJ
  narrows. Also used by the trainer's EQ exercises. Shared by the processor (real
...
- `shared/dsp/CompressorEngine.h` (moved in ADR 040; the trainer's
  compression exercise runs it too) — custom feed-forward
...
- `shared/dsp/ReverbEngine.h` (moved in ADR 040; the trainer's reverb
  exercise runs it too) — rewritten in ADR 037: Room/Hall
...
- `shared/learning/TrainingModule.{h,cpp}` + `shared/audio/LessonAudioBed.{h,cpp}` +
...
- `tests/SharedEnginesTest.cpp` (ADR 040) — the matched bell against the
  analog bell, cut = 1/boost and stable, ADAA antiderivatives and the
  aliasing they remove, the exercise's own sound levelled to the noise,
  boosts-first, and weighted target draws.
